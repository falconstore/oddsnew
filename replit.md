...
### Grupo bônus "Área do Aluno" (desde 2026-04-23)
Sistema agora suporta um SEGUNDO grupo Telegram opcional ("Área do Aluno") em paralelo ao VIP de odds. Toda a feature é gateada pelo env `TELEGRAM_TRIAL_BONUS_CHAT_ID` (formato `-100…`) nas Edge Functions. **Sem esse env, o comportamento é 100% idêntico ao anterior** — único grupo VIP, sem 2º botão, sem badges de bônus.

**Schema** (`supabase/migrations/20260423_trial_bonus_group.sql`): adiciona em `trial_leads` 3 colunas nullable + partial index — `bonus_invite_link`, `bonus_entered_at`, `bonus_removed_at`. Aditivo puro, sem defaults: leads pré-bônus ficam com NULL para sempre (lidos como "Bônus indisponível" no painel).

**Regra de ativação INALTERADA**: SÓ o `chat_member` no grupo VIP seta `status='active'` + `entered_at` + `expires_at=now+7d`. JOIN no bônus apenas preenche `bonus_entered_at` — nunca toca `status`/`expires_at`. Se o lead jamais entrar no VIP, ele continua `pending` mesmo que tenha entrado no bônus. Bônus NÃO é uma porta dos fundos pra ativar trial.

**`trial-signup`**: cria invite_link no VIP (obrigatório) e tenta criar um 2º invite_link no bônus (24h/1-uso), gravando em `bonus_invite_link`. Bônus é **best-effort**: se o bot não for admin no grupo bônus ou der qualquer erro Telegram, signup continua, retorna só o VIP, e o lead simplesmente não verá o 2º botão. Resposta JSON ganha o campo `bonus_invite_link` (string ou null).

**`trial-webhook` /start (Caso 0)**: quando `lead.bonus_invite_link` existe, renderiza 2 botões inline — linha 1 `🚀 Entrar no grupo VIP`, linha 2 `🎁 Entrar na Área do Aluno (bônus)`. Sem `bonus_invite_link`, mostra só o VIP (igual ao fluxo antigo). Texto do /start menciona o bônus em uma linha extra quando aplicável.

**`trial-webhook` chat_member (Caso bônus, novo)**: distingue VIP vs bônus pelo `chat.id` do update. No bônus, anti-repeat completo com paridade do VIP: (a) stranger join (sem lead match) → kick; (b) lookup achou lead via `bonus_invite_link` mas `telegram_user_id` é OUTRO user → kick + revoke; (c) lookup achou lead pelo `user_id` mas `inviteLink` recebido não bate com `bonus_invite_link` salvo → marca lead novo como `blocked_repeat` + kick + revoke (cobre o caso "user já tem trial e tentou entrar via link bônus de outro lead"); (d) lead em status bloqueado → kick. JOIN legítimo seta `bonus_entered_at`; LEFT seta `bonus_removed_at`. Logs estruturados específicos: `bonus-stranger-kicked`, `bonus-repeat-kick`, `bonus-blocked-repeat`, `bonus-blocked-kick`, `bonus-joined`, `bonus-left`.

**`trial-cron` / `trial-kick` / `trial-purge` (dual kick)**: na expiração diária e na remoção manual, o bot é instruído a kickar dos DOIS grupos. **VIP é hard-required em cron+kick** (erro VIP retorna 502/marca falha). **Bônus é best-effort em todos** — só `console.warn`, não bloqueia o update no banco. Justificativa: o bot pode perder admin no bônus a qualquer momento, e isso não pode quebrar o fluxo crítico do VIP nem deixar leads "presos". `trial-cron` agora retorna `bonus_kicked` + `bonus_failed` no JSON. Revogação de invite_link aplica nos 2 grupos quando aplicável. **`trial-purge` segue best-effort também no VIP por design** (é a "saída de emergência" do painel — `DELETE FROM trial_leads` sempre roda, mesmo se o Telegram der erro, conforme política existente já documentada acima na seção "Operação & resiliência").

**`trial-diagnose`**: bloco bônus condicional — só reporta quando `TELEGRAM_TRIAL_BONUS_CHAT_ID` está setado. Campos novos no JSON: `has_bonus_chat_id`, `bonus_configured`, `bonus_in_chat`, `bonus_status_in_chat`, `bonus_can_restrict_members`, `bonus_chat_title`, `bonus_chat_type`. Issues equivalentes ao VIP (bot fora do grupo, não é admin, sem permissão "Ban users"). Sem env, todos NULL/false e ZERO issues geradas — backward-compat silenciosa.

**Painel `/trial-admin` (badges por lead)**: na linha logo abaixo de email/whatsapp/@, 2 badges sempre visíveis. **VIP** = verde com `entered_at` formatado se entrou, cinza "VIP pendente" se não entrou ainda. **Bônus** = amarelo com `bonus_entered_at` formatado se entrou, cinza "Bônus pendente" se tem `bonus_invite_link` mas não entrou, ou neutro "Bônus indisponível" se `bonus_invite_link` é NULL (lead pré-feature ou env não estava setado no signup do lead).

**Setup operacional** (ordem importa):
1. Criar/escolher o grupo "Área do Aluno" no Telegram → adicionar `@sharkinhogreen_bot` como admin com permissões **"Ban users"** + **"Invite users via link"**.
2. Pegar o `chat_id` do grupo bônus (formato `-100…`).
3. Supabase Dashboard → Edge Functions → Secrets → setar `TELEGRAM_TRIAL_BONUS_CHAT_ID`.
4. Re-deploy as 6 funções (todas `--no-verify-jwt`): `trial-signup`, `trial-webhook`, `trial-cron`, `trial-kick`, `trial-purge`, `trial-diagnose`.
5. Aplicar a migração `20260423_trial_bonus_group.sql` (SQL Editor ou Management API).
6. /trial-admin → "Diagnosticar Telegram" → confirmar bot admin no bônus + sem issues.

**Para desativar o bônus** sem perder dados: basta remover o env `TELEGRAM_TRIAL_BONUS_CHAT_ID` e re-deployar — o fluxo volta ao single-group automaticamente. Colunas no banco e badges "Bônus indisponível" permanecem, mas inertes (signup deixa de criar link bônus, webhook deixa de renderizar 2º botão, cron/kick deixam de tentar kick no bônus).
