...
- **Lastlink**: External checkout platform integrated for subscription payments and coupon application. The `lastlink-webhook` Edge Function (`supabase/functions/lastlink-webhook/index.ts`) receives Postback events from Lastlink at `https://wspsuempnswljkphatur.supabase.co/functions/v1/lastlink-webhook?token=<LASTLINK_WEBHOOK_TOKEN>` (token stored as Edge Function secret). It saves every raw event to the `lastlink_events` audit table and matches the buyer to a `trial_leads` row by email/order_id/subscription_id, populating `lastlink_order_id`, `lastlink_subscription_id`, `paid_amount`, `paid_currency`, `paid_at`, `payment_method`, `plan_name`, `coupon_code`, `subscription_status`, `canceled_at`, `refunded_at`, `lastlink_last_event`, `lastlink_raw`. On paid events (`Purchase_Order_Confirmed`, `Purchase_Request_Confirmed`, `Recurrent_Payment`) it also forces `status='converted'` to prevent the trial-webhook re-kick path. Migration: `supabase/migrations/20260502_trial_lastlink_payment.sql`.
