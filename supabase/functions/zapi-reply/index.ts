...
function buildAppMessage(email: string, password: string): string {
  return [
    ...
    `🤖 *Android (Chrome):*`,
    `${PWA_URL}`,
    `_Abra o link no Chrome → toque nos três pontinhos ⋮ → "Adicionar à tela inicial" ✅_`,
    ``,
    `🍎 *iPhone (Safari):*`,
    `${PWA_URL}`,
    `_Abra o link no Safari → toque no ícone de compartilhar → "Adicionar à Tela de Início" ✅_`,
  ].join("\n");
}
  ...
  if (step === "awaiting_help_type") {
    ...
    if (text === "help_app") {
      const msg = [
        ...
        "",
        `🤖 *Android (Chrome):*\n${PWA_URL}\n_Toque nos três pontinhos ⋮ → "Adicionar à tela inicial" ✅_`,
        "",
        `🍎 *iPhone (Safari):*\n${PWA_URL}\n_Toque em compartilhar → "Adicionar à Tela de Início" ✅_`,
        "",
