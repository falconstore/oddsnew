...
// Atualize com o link real do grupo VIP do Telegram
const VIP_TELEGRAM_URL = 'https://t.me/+SEU_LINK_VIP_AQUI'
  ...
  return (
    <div className="page-content no-scrollbar px-4">
      ...
      {status === 'active_subscriber' && (
        <a href={VIP_TELEGRAM_URL} target="_blank" rel="noopener noreferrer"
           className="flex items-center gap-3 px-4 py-3 rounded-2xl mb-4 active:scale-[0.98] transition-transform"
           style={{
             background: 'linear-gradient(135deg, rgba(30,222,107,0.18) 0%, rgba(30,222,107,0.08) 100%)',
             border: '1px solid rgba(30,222,107,0.35)',
           }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
               style={{ background: 'rgba(30,222,107,0.2)', border: '1px solid rgba(30,222,107,0.4)' }}>
            <Send size={18} style={{ color: 'hsl(145 80% 52%)' }} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-white leading-tight">Grupo VIP do Telegram</p>
            <p className="text-xs mt-0.5 truncate" style={{ color: 'rgba(255,255,255,0.5)' }}>
              Acompanhe os sinais em tempo real →
            </p>
          </div>
          <span className="text-[10px] font-black px-2 py-1 rounded-lg flex-shrink-0"
                style={{ background: 'rgba(30,222,107,0.25)', color: 'hsl(145 80% 58%)' }}>
            ENTRAR
          </span>
        </a>
      )}
