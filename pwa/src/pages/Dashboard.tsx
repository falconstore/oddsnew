...
import { TrendingUp, Zap, BarChart2, Star, Clock, ChevronRight } from 'lucide-react'
...
function ActiveCard({ p, onClick }: { p: Procedure; onClick: () => void }) {
  const hasKickoff = Boolean(p.kickoff_at)
  ...
  return (
    <div className="flex gap-1 flex-wrap">
      {stats.map(s => {
        ...
        return (
          <div key={s.date} title={`${s.date}: R$${s.lucro.toFixed(0)}`}
...
export function Dashboard() {
  ...
  const totalLucro = todayStats?.lucro ?? 0
  ...
  return (
    <div className="page-content no-scrollbar px-4">
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <KpiCard label="Operações hoje" value={String(todayStats?.total ?? 0)} icon={BarChart2} color="hsl(145 80% 48%)" />
        <KpiCard label="Lucro hoje"
          value={totalLucro >= 0 ? `+R$${totalLucro.toFixed(0)}` : `-R$${Math.abs(totalLucro).toFixed(0)}`}
          icon={TrendingUp} color={totalLucro >= 0 ? 'hsl(145 80% 48%)' : '#f87171'} />
        <KpiCard label="Freebets ganhas" value={String(todayStats?.freebets ?? 0)} icon={Zap} color="#facc15" />
        <KpiCard label="Duplo Green" value={String(todayStats?.dg ?? 0)} icon={Star} color="#a78bfa" />
      </div>
            ...
            <ResponsiveContainer width="100%" height={120}>
              <AreaChart data={allStats} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number) => [`R$${Number(v).toFixed(0)}`, 'Lucro']}
                />
