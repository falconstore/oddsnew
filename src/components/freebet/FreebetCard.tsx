    ...
    >
      <div className="flex flex-col min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          {isFreebet && <Badge className="text-[9px] px-1 py-0 bg-primary/80">FREEBET</Badge>}
        </div>
      ...
      <div className="flex items-center gap-3">
        <div className="text-right">
          <div className={cn("font-mono font-bold", variant === "so" ? "text-amber-500" : "text-emerald-500")}>
            {odd.toFixed(2)}
          </div>
          <div className={cn("text-xs text-muted-foreground", isFreebet && "line-through opacity-60")}>
            {formatBRL(stake)}
          </div>
        </div>
...
export function FreebetCard({ opportunity, matchStarted = false }: FreebetCardProps) {
  ...
  return (
    <Card
      className={cn(
        ...
        matchStarted && "bg-blue-950/30 border-blue-500/40 opacity-75",
      )}
    >
      <CardHeader className="pb-2 space-y-2">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="text-xs text-muted-foreground truncate">{match.league_name}</span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            {matchStarted && <Badge className="text-[9px] px-1.5 py-0 bg-blue-600/80 text-white">INICIADA</Badge>}
